			...
			if(style.background != null){
				if(style.background.image.good()){
					...
					if(bmd.exists(src+tile)){
						...
						bd = getTile(FS.getBitmapData(src),tile,scale);
...
//tf.scrollV++;  
	...
	public override function clear(list:List<DoData>)
	{ 
		for(el in list){ 
			queue.remove(el.o.id);
			forms.remove(el.o.id);
			if(shapes.exists(el.o.id)){
				monitor.removeChild(shapes[el.o.id]);
				shapes.remove(el.o.id);
			}
		}
	}// clear()
